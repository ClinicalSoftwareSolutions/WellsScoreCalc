...
exports.Create = function(_parent) {
    if(!_parent) Ti.API.error("A parent window needs to be passed");
        ...
        var win = Ti.UI.createWindow({title:L('Wells PE score')});
        ...
        tv.addEventListener('click', function(e){
            ...
            if (totalScore<=4) {
                _scoreLabel.text = "Score: " + totalScore + " PE unlikely\nClick for flowchart";
                _scoreLabel.backgroundColor = '#22ee22';                
            }
            else {
                _scoreLabel.text = "Score: " + totalScore + " PE likely\nClick for flowchart";
                _scoreLabel.backgroundColor = '#dd2222';                       
            }
        });
        
        var headerLbl = Ti.UI.createLabel({text: 'PE: Please Select Items',
...
function _viewFlowchart() {
    ...
    var win = Ti.UI.createWindow({title: 'PE Flowchart'});
    ...
    if (_platform === 'android_disable') {
        ...
        webview.html = '<html><head><meta name="viewport" content="width=device-width, initial-scale=' + initialScale + ', maximum-scale=1"></head>'
            + '<body><img src="PE_fc.png" /></body></html>';
        win.add(webview);
    }
